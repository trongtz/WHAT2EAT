...
CUISINE_PATTERNS = {
    "món việt": ["viet", "com nha", "quan com", "bun", "pho", "hu tieu", "banh cuon", "banh xeo"],
...
BUDGET_PATTERNS = {
    "binh_dan": ["gia re", "binh dan", "sinh vien", "tiet kiem"],
...
def parse_query_heuristically(query: str) -> QueryIntent:
    ...
    open_now = True if any(phrase in normalized for phrase in ["dang mo", "mo ngay", "mo khuya"]) else None
...
def _parse_group_size(normalized_text: str) -> int | None:
    digit_match = re.search(r"(\d{1,2})\s*(nguoi|ban)", normalized_text)
    if digit_match:
        return int(digit_match.group(1))

    for word, value in NUMBER_WORDS.items():
        if re.search(rf"\b{word}\s*(nguoi|ban)\b", normalized_text):
            return value
    return None
